VERSION 5.00
Object = "{68670FAD-7A98-11D5-910D-0080C845CEED}#2.0#0"; "TXTBOX.OCX"
...
Private Sub CMDCommit_Click()
    ...
    If (OpenRecordset(rstemp, "SELECT * FROM [user] WHERE [uid]=" & vCurrentUserID, dbOpenDynaset) = True) Then
...
Private Sub CMDDelete_Click()
    If (vCurrentUserID > 0) Then
        If (Messagebox("Delete user, are you sure ?", vbQuestion) = vbYes) Then
            Call Execute("DELETE FROM [user] WHERE UID=" & vCurrentUserID)
        End If
    End If
End Sub
...
Private Sub TXTUserName_KeyPress(KeyAscii As Integer)
    ...
    If (KeyAscii = 13) Then
        ' Load details here
        If (OpenRecordset(rstemp, "SELECT * FROM [user] WHERE [name]='" & TXTUserName.Text & "'", dbOpenDynaset) = True) Then
            If (rstemp.EOF = False) Then
