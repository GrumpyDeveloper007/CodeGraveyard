...
Begin VB.Form FRMConfigEmployee 
   ...
   End
   Begin VB.CheckBox CHKIncludeInReport 
...
Private Sub ClearDetails()
    ...
    TXTName.Text = ""
    CHKIncludeInReport.Value = vbChecked
    
    '' Sync with Net2System
    If (OpenRecordset(rstemp, "SELECT * FROM [USERs]", dbOpenSnapshot)) Then
        Do While (rstemp.EOF = False)
            If (OpenRecordset(rstemp2, "SELECT * FROM [E_Employee] WHERE [USERID]=" & rstemp!userid, dbOpenSnapshot)) Then
                If (rstemp2.EOF = False) Then
                    'update
                Else
                    ' Create
                    Call Execute("INSERT INTO E_Employee (Name,USERID) VALUES (" & cTextField & rstemp!firstname & " " & rstemp!surName & cTextField & "," & rstemp!userid & ")")
                End If
    ...
    If (OpenRecordset(rstemp, "SELECT * FROM E_Employee ORDER BY [Name]", dbOpenSnapshot)) Then
        SaveIndex = GRDEmployee.TopIndex
        Call GRDEmployee.Clear
        If (rstemp.EOF = False) Then
            Do While (rstemp.EOF = False)
                Call GRDEmployee.AddItem(rstemp!Uid & vbTab & rstemp!Name & vbTab & rstemp!Include)
...
Private Sub SetupFieldSizes()
    Dim rstemp As Recordset
    Dim i As Long
    If (OpenRecordset(rstemp, "E_Employee", dbOpenTable)) Then
...
Private Sub CMDAddNew_Click()
    ...
    If (CHKIncludeInReport.Value = vbChecked) Then
        Include = True
    Else
        Include = False
    End If
    If (TXTName.Text <> "") Then
        If (vUid > 0) Then
            ' update
            If (Execute("UPDATE E_Employee Set [name]=" & cTextField & Trim$(TXTName.Text) & cTextField & ", Include=" & Include & " WHERE UID=" & vUid) > 0) Then
                vDataChanged = True
                Call ClearDetails
            End If
        Else
            ' add new
            If (Execute("INSERT INTO E_Employee (Name,Include) VALUES (" & cTextField & Trim$(TXTName.Text) & cTextField & "," & Include & ")") > 0) Then
                vDataChanged = True
                Call ClearDetails
            End If
        End If
...
Private Sub CMDDelete_Click()
    If (GRDEmployee.ListIndex >= 0) Then
        GRDEmployee.Col = 0
        If (Execute("DELETE FROM E_Employee WHERE UID=" & GRDEmployee.ColText) > 0) Then
            vDataChanged = True
...
Private Sub GRDEmployee_Click()
    If (GRDEmployee.ListIndex >= 0) Then
        ...
        GRDEmployee.Col = 2
        If (GRDEmployee.ColText = "False") Then
            CHKIncludeInReport.Value = vbUnchecked
        Else
            CHKIncludeInReport.Value = vbChecked
        End If
