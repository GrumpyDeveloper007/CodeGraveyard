...
Friend Class frmReaderHolder
    ...
    Private Sub GetOutputSegmentSize(ByRef pOutputSegSize As Integer, ByRef pfilename As String, ByRef pLastFileID As Integer, ByRef pLastArticleIndex As Long)
    ...
    Private Sub ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer)
...
fileerror:
    ...
    Private Sub m_cNewsReader_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReader.AddUpdateGroup
    ...
    Private Sub m_cNewsReader_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReader.GetLastArticleForGroup
    ...
    Private Sub m_cNewsReader_GetOutputSegmentSize(ByRef pOutputSegSize As Integer, ByRef pfilename As String, ByRef pLastFileID As Integer, ByRef pLastArticleIndex As Long) Handles m_cNewsReader.GetOutputSegmentSize
    ...
    Private Sub m_cNewsReader_PacketRecieved(ByRef pNumber As Integer) Handles m_cNewsReader.PacketRecieved
        ...
        End If
        MDIMain.LBLPacketNumber0.Text = "Packet =" & pNumber & "(" & m_PacketNumberMax & ")"
    End Sub
    ...
    Private Sub m_cNewsReader_ProcessAllGroupsList() Handles m_cNewsReader.ProcessAllGroupsList
        ...
        For i = 0 To m_MaxGroup - 1
            ...
            If (i Mod 20 = 0) Then
                MDIMain.LBLProgress(0) = i & "-" & m_MaxGroup & "(" & NewGroups & ")"
    ...
    Private Sub m_cNewsReader_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReader.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 0)
    End Sub

    Private Sub m_cNewsReader_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReader.ProcessMissingArticle
    ...
    Private Sub m_cNewsReaderb_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderB.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderb_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderB.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderb_PacketRecieved(ByRef pNumber As Integer) Handles m_cNewsReaderB.PacketRecieved
        If (pNumber > m_PacketNumberMax) Then
            m_PacketNumberMax = pNumber
        End If
        MDIMain.LBLPacketNumber1.Text = "Packet =" & pNumber & "(" & m_PacketNumberMax & ")"
    End Sub
    ...
    Private Sub m_cNewsReaderB_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderB.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 1)
    End Sub

    Private Sub m_cNewsReaderB_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderB.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReaderc_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderC.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderc_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderC.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderc_PacketRecieved(ByRef pNumber As Integer) Handles m_cNewsReaderC.PacketRecieved
        If (pNumber > m_PacketNumberMax) Then
            m_PacketNumberMax = pNumber
        End If
        MDIMain.LBLPacketNumber2.Text = "Packet =" & pNumber & "(" & m_PacketNumberMax & ")"
    End Sub
    ...
    Private Sub m_cNewsReaderC_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderC.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReaderC_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderC.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReaderd_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderD.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderd_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderD.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderd_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderD.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReaderd_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderD.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReadere_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderE.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReadere_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderE.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReadere_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderE.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReadere_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderE.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReaderf_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderF.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderf_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderF.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderf_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderF.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReaderf_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderF.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReaderg_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderG.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderg_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderG.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderg_GetOutputSegmentSize(ByRef pOutputSegSize As Integer, ByRef pfilename As String, ByRef pLastFileID As Integer, ByRef pLastArticleIndex As Long) Handles m_cNewsReaderG.GetOutputSegmentSize
        Call GetOutputSegmentSize(pOutputSegSize, pfilename, pLastFileID, pLastArticleIndex)
    End Sub
    ...
    Private Sub m_cNewsReaderg_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderG.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReaderg_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderG.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
    ...
    Private Sub m_cNewsReaderh_AddUpdateGroup(ByRef pName As String, ByRef pLowArticle As Long, ByRef pHighArticle As Long, ByRef pPostability As String) Handles m_cNewsReaderH.AddUpdateGroup
        ' Store groups for update later
        m_Groups(m_MaxGroup).Name = pName
        m_Groups(m_MaxGroup).LowArticle = pLowArticle
        m_Groups(m_MaxGroup).HighArticle = pHighArticle
    ...
    Private Sub m_cNewsReaderh_GetLastArticleForGroup(ByRef pLastArticleID As Long, ByRef pGroupID As Integer, ByRef pServerID As Integer) Handles m_cNewsReaderH.GetLastArticleForGroup
        Dim rstemp As ADODB.Recordset
        If pServerID = 0 Then
            pLastArticleID = Val(GetServerSetting("GroupMax-" & pGroupID, False))
        Else
    ...
    Private Sub m_cNewsReaderh_ProcessArticle(ByRef pArticleNumber As Long, ByRef psubject As String, ByRef pLineCount As Integer, ByRef pPostDate As Date, ByRef pGroupID As Integer, ByRef pLinesExpected As Integer) Handles m_cNewsReaderH.ProcessArticle
        Call FRMMain.ProcessArticle(pArticleNumber, psubject, pLineCount, pPostDate, pGroupID, pLinesExpected, 2)
    End Sub

    Private Sub m_cNewsReaderh_ProcessMissingArticle(ByRef pArticleNumber As Long, ByRef pGroupID As Integer) Handles m_cNewsReaderH.ProcessMissingArticle
        Call ProcessMissingArticle(pArticleNumber, pGroupID)
    End Sub
